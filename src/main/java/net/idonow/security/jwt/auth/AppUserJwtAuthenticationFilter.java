...
import net.idonow.controller.exception.security.PhoneNumberNotVerifiedException;
...
public class AppUserJwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public AppUserJwtAuthenticationFilter(AuthenticationManager authenticationManager,
    ...
    public Authentication attemptAuthentication(HttpServletRequest request,
        ...
        try (ValidatorFactory validatorFactory = Validation.buildDefaultValidatorFactory()) {
            ...
            Set<ConstraintViolation<AuthenticationRequest>> violations = validatorFactory.getValidator().validate(authenticationRequest);
            if (!violations.isEmpty()) {
                response.setStatus(HttpStatus.BAD_REQUEST.value());
                response.setContentType(APPLICATION_JSON_VALUE);
                response.setCharacterEncoding("UTF-8");
                response.getWriter().println(objectMapper.writeValueAsString(
                        ApiResponseHelpers.unsuccessfulAuthResponse(
                                localeUtils.getLocalizedMessage("error.data-verification"), ApplicationErrorCode.AUTHENTICATION_ERROR
                        )));
                return null;
            }
        } catch (IOException e) {
    ...
    protected void unsuccessfulAuthentication(HttpServletRequest request,
        ...
        if (failed.getClass().equals(BadCredentialsException.class) || (failed.getCause() != null &&
            ...
            } else if (failed.getCause() != null && failed.getCause().getClass().equals(PhoneNumberNotVerifiedException.class)) {
                unsuccessfulAuth = ApiResponseHelpers.unsuccessfulAuthResponse(
                        message, ApplicationErrorCode.PHONE_NUMBER_NOT_VERIFIED
                );
            }
        } else {
